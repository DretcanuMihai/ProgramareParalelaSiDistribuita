...
double OUTPUT_MATRIX[IO_ROWS][IO_COLUMNS];
 ...
 */
void solveConvolution(const matrix_info& inputMatrixInfo, matrix_info& outputMatrixInfo, const matrix_info& kernelMatrixInfo,
	int start, int nrElements) {
	...
	while (remainingElements > 0) {
		...
		OUTPUT_MATRIX[currentRow][currentColumn] = accumulator;
 ...
 */
void solveSequential(const matrix_info& inputMatrixInfo, matrix_info& outputMatrixInfo, const matrix_info& kernelMatrixInfo) {

	const int size = inputMatrixInfo.rows * inputMatrixInfo.columns;
	solveConvolution(inputMatrixInfo, outputMatrixInfo, kernelMatrixInfo, 0, size);
}
 ...
 */
void solveThreaded(const matrix_info& inputMatrixInfo, matrix_info& outputMatrixInfo, const matrix_info& kernelMatrixInfo,
	const int nrThreads) {
	...
	for (int i = 0; i < nrThreads; i++) {
		...
		threads[i] = thread(solveConvolution, ref(inputMatrixInfo), ref(outputMatrixInfo), ref(kernelMatrixInfo),
			start, nrElements);
...
matrix_package readPackage(const string& filename) {
	...
	int rows = inputMatrixInfo.rows;
	int columns = inputMatrixInfo.columns;
	matrix_info outputMatrixInfo{ rows,columns };

	const matrix_info kernelMatrixInfo = readKernelMatrix(inputStream, KERNEL_MATRIX);

	inputStream.close();

	return { inputMatrixInfo, outputMatrixInfo, kernelMatrixInfo };
}
...
void assertContentsEqual(const string& filename1, const string& filename2) {
	...
	//reusing the input/output matrices such that no extra memory is used
	const matrix_info inputMatrixInfo1 = readMatrix(inputStream1, INPUT_MATRIX);
	const matrix_info inputMatrixInfo2 = readMatrix(inputStream2, OUTPUT_MATRIX);
	...
	for (int i = 0; i < rows1; i++) {
		for (int j = 0; j < columns1; j++) {
			assert(INPUT_MATRIX[i][j] == OUTPUT_MATRIX[i][j]);
		}
	}
...
int main(int argc, char** argv) {
	...
	matrix_info outputMatrixInfo = matrixPackage.outputMatrixInfo;
	...
	if (nrThreads > 0) {
		...
		solveThreaded(inputMatrixInfo, outputMatrixInfo, kernelMatrixInfo, nrThreads);
		endWatch = std::chrono::high_resolution_clock::now();

		writeMatrix(OUTPUT_FILE_NAME1, outputMatrixInfo, OUTPUT_MATRIX);
	...
	startWatch = std::chrono::high_resolution_clock::now();
	solveSequential(inputMatrixInfo, outputMatrixInfo, kernelMatrixInfo);
	endWatch = std::chrono::high_resolution_clock::now();

	writeMatrix(OUTPUT_FILE_NAME2, outputMatrixInfo, OUTPUT_MATRIX);
