...
void solveConvolution(const matrix_info& inputMatrixInfo, matrix_info& outputMatrixInfo, const matrix_info& kernelMatrixInfo,
	const int start, const int nrElements) {


	double** const inputMatrix = inputMatrixInfo.matrix;
	double** outputMatrix = outputMatrixInfo.matrix;
...
matrix_package readPackage(const string& filename) {
	...
	double** outputMatrix = new double* [rows];
	for (int i = 0; i < rows; i++) {
		outputMatrix[i] = new double[columns];
	}

	matrix_info outputMatrixInfo{ rows,columns,outputMatrix };
...
int main(int argc, char** argv) {
	...
	if (nrThreads > 0) {
		...
		writeMatrix(OUTPUT_FILE_NAME1, outputMatrixInfo);
	...
	writeMatrix(OUTPUT_FILE_NAME2, outputMatrixInfo);
